...
We'll use standard filtering parameters: `maxN=0` (DADA2 requires no Ns), `truncQ=2`, `rm.phix=TRUE` and `maxEE=2`. The `maxEE` parameter sets the maximum number of "expected errors" allowed in a read, which is [a better filter than simply averaging quality scores](https://www.drive5.com/usearch/manual/exp_errs.html).
...
In comparison, [the mothur pipeline finds 34 OTUs in this Mock community sample](https://mothur.org/wiki/miseq_sop/#assessing-error-rates). DADA2 infers sequence variants exactly instead of fuzzy 97\% OTUs, and outputs fewer false positives to boot!
