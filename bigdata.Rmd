---
title: "A DADA2 workflow for Big Data (1.4 or later)"
...
*This workflow takes advantage of functionality added in the 1.4 release. The [workflow for the earlier 1.2 version of the dada2 R package](bigdata_1_2.html) is also available.*
...
*These issues are discussed in more detail in our open-access ISMEJ paper ["Exact sequence variants should replace operational taxonomic units in marker gene data analysis"](https://doi.org/10.1038/ismej.2017.119)*.
...
# Assign taxonomy
tax <- assignTaxonomy(seqtab, "path/to/silva_nr_v138_train_set.fa.gz", multithread=TRUE)
# Write to disk
...
**Hardware:** A [general compute node](https://www.sherlock.stanford.edu/docs/) with 16 cores and 64GB of memory.

**Running times:** The Sample Inference step (16 cores, 64GB) takes from 2-12 hours, with running times increasing with lower run quality and higher diversity samples. Paired-end sequencing takes twice as long, as sample inference is run independently on the forward and reverse reads before merging (see [tutorial](tutorial.html) and the [paired-end version of the big data workflow](bigdata_paired.html)). The Filtering and Chimera/Taxonomy steps generally take significantly less time than Sample Inference.
