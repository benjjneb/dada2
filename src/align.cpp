...
{
  struct nwinfo_s * nw = (struct nwinfo_s *) malloc(sizeof(struct nwinfo_s));
  nw->dir = 0;
  nw->dir_alloc = 0;
  nw->hearray = 0;
  nw->hearray_alloc = 0;
  ...
  if (qlen * dlen > nw->dir_alloc)
    {
      ...
      nw->dir = (char *) realloc(nw->dir, (size_t)nw->dir_alloc);
    }
  ...
  if (need > nw->hearray_alloc)
    {
      ...
      nw->hearray = (long *) realloc(nw->hearray, (size_t)nw->hearray_alloc);
    }
...
///!      h += getscore(score_matrix, dseq[j], qseq[i]);
      h += score_matrix[dseq[j]-1][qseq[i]-1];
  ...
  char * cigar = (char *) malloc((size_t)(qlen + dlen + 1));
...
///!      score += getscore(score_matrix, dseq[j-1], qseq[i-1]);
      score += score_matrix[dseq[j-1]-1][qseq[i-1]-1];
  ...
  cigar = (char*) realloc(cigar, (size_t)(cigarlength+1));
