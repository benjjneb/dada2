...
**Start here:** The [DADA2 walkthrough on paired end Illumina Miseq data](tutorial.html) goes through a typical workflow in which raw amplicon sequence data is processed into the table of biological sequence variants present in each sample.

The [DADA2 workflow on Big Data](bigdata.html) goes through workflow optimized to run on large datasets (100s of millions of reads).
...
* The computational costs of DADA2 can scale linearly with the number of samples, and DADA2's memory requirements can remain essentially flat.
