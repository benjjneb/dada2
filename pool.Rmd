...
The `dada(...)` function implements the high-resolution sample-inference at the core of the dada2 package. Because `dada(...)` resolves amplicon sequence variants (ASVs) exactly, it is possible to analyze samples separately before combining them together in a final sequence table. However, the `dada(...)` function also allows samples to be pooled together for the error rate estimation and sample inference steps. Here we demonstrate that functionality, and discuss the pros and cons of pooling.
...
```{r}
system.time(dd.sep <- dada(drps, err=dd[[1]]$err_out))
```

Now we pool the samples for sample inference:
```{r}
system.time(dd.pool <- dada(drps, err=dd[[1]]$err_out, pool=TRUE))
```
