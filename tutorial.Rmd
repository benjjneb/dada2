...
system(paste0("mkdir ", path, "filtered/"))
filtFs <- paste0(path, "filtered/", sample.names, "_F_filt.fastq.gz")
filtRs <- paste0(path, "filtered/", sample.names, "_R_filt.fastq.gz")
# Filter
...
To perform this joint inference with `dada` we pass it the `selfConsist=TRUE` flag (if `selfConsist=FALSE` it simply uses the provided error rates). As in many optimization problems, the algorithm must begin with an initial guess. For this we tell the algorithm to use the maximum possible error rates in this data (the error rates if only the most abundant sequence is correct and all the rest are errors from it) as it is better to start with error rates that are too high than too low. This is indicated by setting `err=NULL` in the `dada` function call.
